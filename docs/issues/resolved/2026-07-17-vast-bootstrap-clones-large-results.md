...
- 2026-07-17 — Resolved in `1c52d39` (PR closed unmerged). Bootstrap uses a
  depth-one, blob-filtered sparse checkout and fetches only the requested ref.
- 2026-07-18 — Landed on `main` and re-verified on a live Vast host: clone +
  checkout finished in seconds, worktree was ~2.3 MiB before `uv sync`, root
  `results/` was absent, and CUDA torch validation succeeded.
