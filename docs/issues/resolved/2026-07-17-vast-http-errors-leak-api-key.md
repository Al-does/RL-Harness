...
- 2026-07-17 — Resolved in `1c52d39` (PR closed unmerged) via
  `devops/vast/redaction.py` at the `VastClient` / self-destruct boundaries.
  SDK errors and on-box teardown logs redact credential query fields and known
  secret values; wrapped SDK failures suppress the original secret-bearing
  exception chain.
- 2026-07-18 — Landed on `main`; synthetic 410 reproduction confirms
  `api_key=<REDACTED>` and the sentinel secret is absent from the error.
