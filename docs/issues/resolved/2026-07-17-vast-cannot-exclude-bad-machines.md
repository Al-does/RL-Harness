...
- 2026-07-17 — Resolved in `1c52d39` (PR closed unmerged). `up` supports
  `--offer-id`, `--exclude-machine`, and concurrent readiness workers.
- 2026-07-18 — Landed on `main` with follow-ups from live rentals: explicit
  `--regions` is now a hard country filter; hosts that miss readiness are
  destroyed and replaced from the ranked pool, also excluding the failed
  public IP so sibling offers on the same farm are skipped. Live US rental
  reached `/root/.vast_ready` with CUDA after excluding known-bad machines.
